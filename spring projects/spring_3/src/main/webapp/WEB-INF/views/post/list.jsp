...
<body>
	...
	<c:if test="${pm.cri.co_num != 0 }">
					...
					<tr>
						...
						<td>
							<c:url var="url" value="/post/list/${pm.cri.co_num }">
								<c:param name="type" value="po_me_id"></c:param>
		...
		<!-- url 마지막 부분을 작성하지 못했다. 그리고 button 으로 했었다. -->
		<a href="<c:url value="/post/insert/${pm.cri.co_num }"/>" class="btn btn-outline-info mb-3 btn-insert"> 글 쓰기</a>
		
		<form class="input-group mb-3" action="<c:url value="/post/list/${pm.cri.co_num }"/>">
			<select class="form-control" name="type">
				<c:if test="${pm.cri.type == '' }">selected</c:if>
