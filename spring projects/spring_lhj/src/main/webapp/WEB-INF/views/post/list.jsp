...
<body>
	...
	<c:if test="${pm.cri.co_num ne 0 }">
		...
		<!--
		<button class="btn btn-outline-info mb-3"> 게시글 등록 </button>
		버튼 태그가 아니라 a 태그를 이용하여, 그 안에 클래스를 이용해서 버튼 모양은 만들어야 한다.
		 -->
		
		<!-- 게시글을 등록할 때, 어느 커뮤니티의 게시글인지 알아야 한다. 그래서 ?co_num=${pm.cri.co_num } 를 입력해줘야 한다. -->
		
		<a href="<c:url value="/post/insert?co_num=${pm.cri.co_num }"/>" class="btn btn-outline-info mb-3 btn-insert"> 게시글 등록</a>
