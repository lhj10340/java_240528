...
public class PhoneEx01 {
	...
	// 클래스(정적) 멤버 변수로 Scanner를 만들면 어디에서든 매개변수로 넘기지 않고 사용이 가능하다.
	public static Scanner scan = new Scanner(System.in);

	public static void main(String[] args) {
		// 반복문 안에 들어가면 조건식에서 사용하지 못하기 때문에 변수를 여기에 선언해야한다.

		final int EXIT = 5;
		int menu = EXIT + 1; // 반복문을 위해 EXIT 가 아닌 값으로 바꿔주어야 한다, 아니면 실행이 안된다.
		int count = 0;
		...
		do {
			...
			try {
				...
				System.out.println("메뉴를 잘못 했습니다.");
				//메뉴를 잘못 선택했을 때, 입력 버퍼에 남은 값을 제거한다.
				scan.next();
			} catch (Exception e) {
	...
	// 메소드 안에 메소드를 넣을 수는 없다.
	// 메소드 생성 시, 괄호 실수하는 경우가 잦으니 유의.
	...
	/** 기능 : 연락처 리스트에 연락처가 몇 번지에 있는지 알려주는 메소드
	 * @param list 연락처 리스트
	...
	public static void update(Contact[] list, int count) throws Exception {
		...
		// 연락처 리스트에서 이름과 일치하는 연락처를 번지 + 1과 함께 출력. ( 0번지의 경우 1번 )
		...
		// 생성된 객체가 중복된 번호이면 안내문구를 출력하고 아니면 객체를 추가
	...
	public static int delete(Contact[] list, int count) {
		...
		// 번호가 유효한지 확인해서 유효하지 않다면 안내문구 출력 후, 종료.
...
class Contact {
	...
	// private 는 접근이 안되는 제어자로 다른 곳에서 사용하기 위하여 getter / setter 가 필요.
	...
	//hashcode equals 는 전화번호가 같으면 등록이 되면 안되고, 수정할 때도 있는 번호는 등록되면 안되기 때문에
	// equals 를 오버라이딩 하면 같은 번호인지 아닌지 비교하기가 쉽다.
	@Override
	public int hashCode() {
	...
	// ToString 을 오버라이딩하면 이름 : 번호 형태의 문자열이 필요할 때, 사용하기가 좋다. ( 여러번 필요하기 때문에 )
	@Override 
	public String toString() {
		//홍길동 연락처 : 010-1231-2333
		return name + " : " + number;
	}
	...
	public void setNumber(String number) throws Exception {
		// 주어진 번호가 전화 번호 형태가 아니면 예외를 발생시키고 맞으면 번호에 저장.
		// 010-1234-5678 또는 02-123-4567 또는 041-1234-5678 형태의 문자열을 처리하기 위한 정규표현식
		String regex = "^\\d{2,3}-\\d{3,4}-\\d{4}$";
		if(!Pattern.matches(regex, number)) {
			// runtime Exception 또는 그의 자식 클래스가 아니라면 예외 선언을 해야한다.
...
// 이름과 연락처를 포함한 클래스를 생성하는 것이 정보의 관리에 좋을 것. 아니면 각각의 배열을 만들어 관리를 해야해서 ...
