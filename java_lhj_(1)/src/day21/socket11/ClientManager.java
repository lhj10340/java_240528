...
public class ClientManager implements Program {
	...
	private void update() {
		...
		// 수정
		// 새로운 연락처가 리스트에 있으면 안내문구 출력 후, 종료. ( 단, 현재 선택한 연락처가 아닌 연락처 중에서 )

		// 전체 탐색
		for(Contact tmp : list) {
			// 리스트에서 가져온 객체와 위에서 가져온 객체가 같지 않으면 건너뛴다.
			if(tmp == contact) {
				continue;
			}
	...
	public void load() {
		...
		try {
			...
			// oos 를 이용해서 load 문자열 전송
			oos.writeUTF("load");
			oos.flush();
			
			// ois 를 이용해서 연락처 리스트를 저장
