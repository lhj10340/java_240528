...
# abc123회원이 id: abc123, pw: abc1234로 로그인 시도했을 때, 실행해야하는 쿼리 (틀린 비밀번호를 입력한 상황)
# 로그인을 실패했다, 실패횟수의 증가
UPDATE MEMBER SET ME_FAIL = ME_FAIL + 1 WHERE ME_ID = 'abc123';

# abc123회원이 로그인 시도가 성공했을 때, 실행해야하는 쿼리
# 실패 후, 다시 성공을 했으니 로그인 실패횟수 초기화
UPDATE MEMBER SET ME_FAIL = 0 WHERE ME_ID = 'abc123';

# abc123회원이 비밀번호 찾기를 시도해서 인증코드가 123asd가 발급되었을 때, 실행해야하는 쿼리
# 인증코드가 발급이 되었으니 5분의 유효 시간을 걸어야 한다? ( 내 생각 )
...
# 변경된 비밀번호로 회원정보를 수정해주는 쿼리가 필요하다.
UPDATE MEMBER SET ME_PW = 'abc1234' WHERE ME_ID = 'abc123';
# 인증코드를 통해 비밀번호를 변경했으니, 이제 인증코드를 삭제해야한다.
