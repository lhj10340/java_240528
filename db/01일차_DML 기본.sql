...
# value는 한 번에 한 행을 추가할 때 사용한다.
# 속성의 개수가 값의 개수와 같아야 한다.
...
# values는 한 번에 한 개 이상의 행을 추가할 때 사용한다.
# 속성의 개수가 값의 개수와 같아야 한다.
...
# 테이블명 옆에 속성명을 생략한 경우 table 생성시 추가했던 속성 순서대로 값들을 넣어주어야 한다.
# 전체 속성 개수만큼 값들을 넣어주어야 한다.
...
# = : 같다, != : 다르다, <> : 다르다, is null : null이면, is not null : null아 아니면
# and : ~ 하고, or : ~ 이거나
# and와 or의 우선순위가 다르다. and가 높다.
# 날짜는 'yyyy-MM-dd hh:ss:mm' 형태의 문자열과 비교 가능. =, >=, <=, !=를 활용할 수 있다.
update student set name = '또치' where student.studentNum = 2;

# 워크 벤치에서는 안전하게 수정/삭제할 수 있게 하기위해 기본키가 아닌 조건으로
# 수정/삭제하려고 하면 수정/삭제를 하지 못하도록 막는다.
# 해결방법 : Edit > preferences... > SQL Editor 
# 			> Safe updates 체크박스 해제 후 ok 클릭. 
#				( 저장 후, 인스턴스<왼쪽 상단>를 나갔다가 들어와야 한다. )
...
# delete는 데이터만 지으고 설정은 그대로 유지 - auto_increment 시작 숫자
# truncate는 데이터 뿐만아니라 설정도 초기로 돌린다 - auto_increment 숫자가 1에서 시작
# truncate는 다른 테이블이 참조하고 있는 경우 초기화가 되지 않는다.
...
# 1학년 1반 1번 홍길동 학생의 1학년 1학기 국어 성적을 중간 100, 기말 90, 수행 90으로 성적을 추가
	insert into student.score(midTerm, finalTerm, performance, studentNum, subjectNum) value(100, 90, 90, 1, 1);
delete from score;
	insert into student.score(midTerm, finalTerm, performance, studentNum, subjectNum) value(100, 90, 90, 1, 1);
# 위 쿼리가 실행되면 기본키가 2(이전 기본키가 1이었다고 가정)인 데이터가 추가된다.
truncate table score;
	insert into student.score(midTerm, finalTerm, performance, studentNum, subjectNum) value(100, 90, 90, 1, 1);
# 위 쿼리가 실행되면 기본키가 1인 데이터가 추가된다.
