...
# 워크벤치에서 DB명 우클릭 Drop Schemas... 클릭 후, Drop now
drop database if exists student;

# 데이터베이스 생성
# if not exists를 추가하는 이유?
# => 생성하려는 DB가 이미 있는 경우 건너뛰기 위해서
# => if not exists가 없는 경우 생성하려는 DB가 있으면 에러가 발생해서 이후 SQL문이 실행되지 않는다.
# MySQL에서는 database와 schema를 같은 것으로 본다.
...
# 데이터베이스 수정 - 이름은 수정 안된다, 문자 집합을 수정
# ALTER SCHEMA DB명 기본문자집합 바꿀문자집합 기본COLLATE 바꿀COLLATE
# 워크스페이스에서 DB명에 호버 후, 2번째 아이콘 클릭, 원하는 문자집합과 collate를 선택 후, apply
...
# auto_increment는 기본키만 적용 가능하다.
use student;
drop table  if exists student.student;
# 선택한 데이터베이스에 따라서 엉뚱한 곳에 생성이 될 수 있다.
...
# 딱 지정을 해서 만들어서 엉뚱한 곳에 생성되지 않는다.
# ` 를 반드시 써야하는 경우가 있는데, table의 이름이 특정 키워드로 되어있는 경우.
