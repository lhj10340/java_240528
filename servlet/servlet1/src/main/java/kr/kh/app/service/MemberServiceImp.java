...
public class MemberServiceImp implements MemberService {
	...
	public boolean signup(LoginDTO member) {
		...
		// 아이디 중복 검사해서 있으면 false 를 리턴
		if(memberDao.selectMember(member.getId()) != null) {
			return false;
		}
		// 유효성 검사 실패 시, false 를 리턴
		...
		// 
		try {
			// 이메일이 중복되면 추가할 수 없어서 (unique 설정) 예외 발생
	...
	public MemberVO login(LoginDTO member) {
		...
		// dao 에게 아이디를 주면서 아이디와 일치하는 회원 정보를 가져오라고 시킨다. ( MemberVO 를 입력하지 못했다, selectMember 를 입력하지 못 했다. )
		MemberVO user = memberDao.selectMember(member.getId());
		// 가져온 회원 정보가 null 이면 null 을 리턴
		if( user == null ) {
			return null;
		}
		// 비밀번호와 가져온 회원정보의 비번이 같으면 가져온 회원 정보를 리턴
